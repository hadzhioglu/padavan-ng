...
function rescan(){
	...
	url: "/apply.cgi",
	data: {
	'action_script': "script/_ezscript getAPSite2g "
	},
	dataType: "json",
	error: function() {
...
function setWDSAP(o, i) {
	...
	document.form.rt_sta_ssid.value = getStrA(Base64.decode(wds_aplist[i][1]));
	}
	if (parseInt(getStrA(wds_aplist[i][0])) > 0){
	document.form.rt_channel.value = getStrA(wds_aplist[i][0]);
	}
	if (getStrA(wds_aplist[i][3]).indexOf('NONE') != -1 || getStrA(wds_aplist[i][3]).indexOf('OPEN') != -1){
	document.form.rt_sta_auth_mode.options[1].selected = 0;
	document.form.rt_sta_auth_mode.options[2].selected = 0;
	document.form.rt_sta_auth_mode.options[0].selected = 1;
	change_sta_auth_mode(1)
	}
	if (getStrA(wds_aplist[i][3]).indexOf('WPA2') != -1){
	document.form.rt_sta_auth_mode.options[0].selected = 0;
	document.form.rt_sta_auth_mode.options[1].selected = 0;
	document.form.rt_sta_auth_mode.options[2].selected = 1;
	change_sta_auth_mode(1)
	}else{
	if (getStrA(wds_aplist[i][3]).indexOf('WPA1') != -1 || getStrA(wds_aplist[i][3]).indexOf('WPAPSK') != -1){
	document.form.rt_sta_auth_mode.options[0].selected = 0;
	document.form.rt_sta_auth_mode.options[2].selected = 0;
	document.form.rt_sta_auth_mode.options[1].selected = 1;
	change_sta_auth_mode(1)
	}
	}
	}
...
<div class="wrapper">
	...
	<table width="100%" align="center" cellpadding="4" cellspacing="0" class="table" id="WDSAPList_select" style="margin-bottom: 0px;">
                                    ...
                                    <table class="table">
                                        <tr>
                                            <td id="col_goto5" width="33%" style="margin-top: 10px; border-top: 0 none;">
